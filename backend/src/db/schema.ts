...
export const seedDefaults = async (queryFn: (sql: string, params?: any[]) => Promise<any>) => {
    await queryFn("INSERT INTO settings (key, value) VALUES ('businessName', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify('Chicken Nation')]);
    await queryFn("INSERT INTO settings (key, value) VALUES ('branchName', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify('Main Branch')]);
    await queryFn("INSERT INTO settings (key, value) VALUES ('currency', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify('XAF')]);
    await queryFn("INSERT INTO settings (key, value) VALUES ('language', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify('en')]);
    await queryFn("INSERT INTO settings (key, value) VALUES ('theme', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify('dark')]);
    await queryFn("INSERT INTO settings (key, value) VALUES ('deliveryFee', $1) ON CONFLICT (key) DO NOTHING", [JSON.stringify(100)]);
};
